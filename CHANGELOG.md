...
wildcard qualifier, `42712` for an alias declared twice, and `42702` for a reference — bare, qualified, or the `s.*`
that would emit it — to a name a sub-select's AUTHOR-WRITTEN select list carries twice (`SELECT id`
/ `SELECT s.id` / `SELECT s.*` over `(SELECT a.id, b.id …) s`). That last one turns on the SELECT
LIST, not on the schema, and it turns on it in ONE direction: the refusal is armed unless the
entry's select list is ENTIRELY wildcards — one or more `*` / `q.*` items and nothing else. A list
that MIXES them (`SELECT *, b.id`) is author-written and keeps its `42702`, because the author
named `b.id` beside a `*` that already carried it. An entirely-wildcard list does NOT: the
duplicate in `(SELECT * FROM a NATURAL JOIN b) j` is an artifact of THIS engine's output shape — we
do not merge the shared output column of a `NATURAL`/`USING` join, so `j` carries `id` twice where
PostgreSQL's `j` carries it once — and `j.id`, the bare `id` and `j.*` therefore read the FIRST of
the two slots, which is the column the sub-select actually returns and the answer v4.31.1 gave.
Neither does a STORED relation whose own schema repeats a name — a MATERIALIZED VIEW over such a
join — because nothing at the reference site wrote that name twice; those columns come from the
catalog, not from a select list in the statement being planned. And a name a BARE `*` expands to is
not resolved at all: `*` names no column, so it can draw no verdict. Every other sub-select and view column resolves through its
...
| `FROM t JOIN (SELECT a.id, b.id FROM …) s ON s.id = t.id` (an AUTHOR-WRITTEN select list carrying `id` twice) | runtime error | `42702` for `s.id`, for the bare `id` and for `s.*` (which would emit the name twice and read one slot for both) — every other `s.col` resolves, and a name a bare `*` expands to is not resolved at all; `AS s(w, x, y, z)` renames the output positionally (a shorter list renames the first columns). Over an entry the ENGINE or the CATALOG built — an entirely-wildcard select list, or a materialized view's stored schema — none of the three is refused: all read the first slot, as v4.31.1 did |
...
| `SELECT id FROM (SELECT a.id, b.id …) s` / `SELECT s.id FROM …` / `SELECT s.* FROM …` (a name one derived entry's AUTHOR-WRITTEN select list carries twice) | the bare `id` read the first slot; the qualified `s.id` and `s.*` were the runtime miss `Column 's.id' not found in schema` that the derived-table row above fixes | `42702` for all three. This is decided by the SELECT LIST, not by the schema, and only a list that is ENTIRELY wildcards disarms it: a MIXED list (`SELECT *, b.id`) stays `42702`, while `(SELECT * FROM a NATURAL JOIN b) j` — and a MATERIALIZED VIEW whose stored schema repeats the name — resolve `j.id`, the bare `id` and `j.*` to the FIRST slot, exactly as v4.31.1 answered them |
| `FROM a JOIN b USING (col)` (every join type) | a CARTESIAN PRODUCT — `JoinConstraint::Using` fell to the planner's `_ => None` and was not lowered at all, so the join ran with NO condition | the equi-join. `NATURAL` and `USING` now share ONE lowering: one `=` per shared column, BOTH operands UNQUALIFIED (`Column{None, c} = Column{None, c}`) — which is what `NATURAL` has always emitted. What makes it an equi join rather than a tautology is the join key binder's rule for an all-unqualified `=` term: only a QUALIFIED double fit is an alias collision, so an all-bare term is keyed in the NATURAL ORDER (lhs → left input, rhs → right input) instead of being declined and re-resolved to one slot. A `USING` column either side does not carry is `42703` (PostgreSQL), never a silent cross join. Pinned three and four tables deep, INNER and LEFT, on both executor families and over the PostgreSQL wire (there is no MySQL-wire pin for `NATURAL` or `USING`) |
...
`gh29_c7_*` / `gh29_c8_*` / `gh29_c10_*` / `gh29_c11_*` / `gh29_c12_*` wire tests (the `gh29_c7_*` and `gh29_c8_*` blocks drive
...
term are unqualified, so each side is keyed on the FIRST column of that name it carries. Under
`INNER` that is PostgreSQL's merged value because both contributors hold the same value; under
`LEFT` it is too, because PostgreSQL merges to the LEFT contributor — which is the first column —
even though on an unmatched left row the RIGHT contributor is NULL and the two therefore do NOT
hold the same value. Under `RIGHT` and `FULL` they part company. The rule is "the first column",
never "the merged one". Three cases where they differ: a chained join whose left input tops out in
a `RIGHT` or `FULL` join (PostgreSQL merges to the right contributor / `COALESCE(left, right)`),
so `a NATURAL RIGHT JOIN b NATURAL JOIN c` can lose or NULL-extend rows PostgreSQL matches; a
reference to the shared name over a wildcard side whose body is topped by a `NATURAL RIGHT`/`FULL`
join (`SELECT j.id FROM (SELECT * FROM a NATURAL RIGHT JOIN b) j`), where the first slot is the
NULL-extended left contributor and we answer NULL where PostgreSQL — which MERGES that column
rather than calling it ambiguous — answers with the right contributor's value
(`a_right_or_full_topped_wildcard_body_answers_null_where_postgresql_merges`); and a left input
whose two same-named columns were never equated AT ALL (`a JOIN b ON a.x = b.x`, both carrying
`id`, then a `NATURAL JOIN` on `id`), where PostgreSQL has two distinct `id` columns and calls the
name ambiguous. This is what the engine does
...
at all. The same first-column rule is what `j.id`, the bare `id` and `j.*` over a wildcard-expanded side
resolve to (see the `SELECT id FROM (SELECT a.id, b.id …) s` row above), and what a reference to a
materialized view's doubled stored column resolves to: where PostgreSQL would call that ambiguous —
or, for a `RIGHT`/`FULL`-topped body, would MERGE it and answer with a value we return as NULL — we
answer with the first slot, which is what v4.31.1 answered and what `SELECT * FROM j` itself
returns. Merging the output column is a change of output arity, its own contract move, and
