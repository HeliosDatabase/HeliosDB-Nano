...
fn main() {
    ...
    db.execute("INSERT INTO users (c1) VALUES ('Javier')")
        .expect("INSERT 1");
    ...
    db.execute("UPDATE users SET c1='Pedro' WHERE c1='Ueli'")
        .expect("UPDATE");

    // Dump keys to see what happened
    let prefix_bytes = b"data:users:";
    let db_arc = db.storage.db();
    let iter = db_arc.iterator(rocksdb::IteratorMode::From(
        prefix_bytes.as_ref(),
