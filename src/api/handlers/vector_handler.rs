...
pub struct CreateVectorStoreRequest {
    ...
    pub metric: String,
    /// Index type (hnsw, persistent_hnsw)
...
pub async fn create_store(
    ...
    let store = state
        .db
        .create_vector_store_with_options(&req.name, req.dimensions as u32, &req.metric, &req.index_type)
...
pub async fn insert_vectors(
    ...
    let ids = req
        .ids
        .unwrap_or_else(|| req.vectors.iter().map(|_| uuid::Uuid::new_v4().to_string()).collect());
    ...
    let inserted_ids = state
        .db
        .insert_vectors_with_options(&store_name, Some(ids.clone()), req.vectors, req.metadata, req.namespace)
...
pub async fn upsert_vectors(
    ...
    let metadata: Option<Vec<HashMap<String, serde_json::Value>>> = if req.vectors.iter().any(|v| v.metadata.is_some())
    {
        Some(
            req.vectors
                .iter()
                .map(|v| v.metadata.clone().unwrap_or_default())
                .collect(),
        )
    } else {
        None
    ...
    state
        .db
        .upsert_vectors_with_options(&store_name, ids, values, metadata, req.namespace)
...
pub async fn search_vectors(
    ...
    let results: Vec<_> = raw_results
        .into_iter()
        .map(|result| crate::api::models::VectorSearchResult {
            id: result.id,
            score: result.score,
            values: result.vector,
            metadata: result.metadata.map(|metadata| serde_json::json!(metadata)),
        })
        .collect();
...
pub async fn text_search(
    ...
    let raw_results = state
        .db
        .text_search(&req.text)
        .map_err(|e| ApiError::internal(format!("Text search failed: {}", e)))?;

    let results: Vec<_> = raw_results
        .into_iter()
        .map(|id| crate::api::models::VectorSearchResult {
            id,
            score: 0.0,
            values: None,
            metadata: None,
...
pub async fn store_texts(
    ...
    let ids = req
        .ids
        .unwrap_or_else(|| req.texts.iter().map(|_| uuid::Uuid::new_v4().to_string()).collect());

    if ids.len() != req.texts.len() {
        return Err(ApiError::bad_request("Number of IDs must match number of texts"));
    }

    let _stored_ids = state
        .db
        .store_texts(&store_name, req.texts.clone())
...
pub async fn hybrid_search(
    ...
    let raw_results = state
        .db
        .hybrid_search(&store_name, req.text.as_deref().unwrap_or(""), req.top_k)
        .map_err(|e| ApiError::internal(format!("Hybrid search failed: {}", e)))?;

    let results: Vec<_> = raw_results
        .into_iter()
        .map(|(id, score)| crate::api::models::VectorSearchResult {
            id,
            score,
            values: None,
            metadata: None,
