...
pub(crate) struct RangeEntry {
    ...
    /// GH#29 (c12, M1/M2/M3): do this entry's output columns come from a
    /// select list the AUTHOR WROTE OUT in the statement being planned?
    ///
    /// True only for a sub-select, CTE or view whose select list NAMES at
    /// least one column of its own. False for every entry whose column list
    /// the ENGINE or the CATALOG decided: a base table, a MATERIALIZED VIEW
    /// (its columns come from the stored schema — the reference site could not
    /// have written the name twice), a table function, a DML target, and a
    /// body whose select list is ENTIRELY wildcards (`SELECT *`,
    /// `SELECT a.*, b.*`). A list that MIXES a wildcard with a written item
    /// (`SELECT *, b.id`) is written-out: the author named `b.id` themselves.
    ///
...
impl RangeEntry {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            runtime_qualifier,
            schema,
            written_select_list: false,
        }
    }

    /// Mark this entry's output list as written out by the author — see
    /// [`Self::written_select_list`]. A builder rather than a `new` argument
    /// so the call sites that CANNOT have an author-written select list (base
    /// tables, materialized views, table functions, DML targets) keep the
    /// safe default, `false`, without naming it.
    pub(crate) fn with_written_select_list(mut self, written_select_list: bool) -> Self {
        self.written_select_list = written_select_list;
        self
    }
    ...
    /// raw schema question only: whether a reference to such a name — written
    /// (`s.id`, the bare `id`) or a `s.*` that would emit it twice — is
    /// REFUSED is [`Self::duplicate_is_ambiguous`], which also asks WHO built
    /// the list. Every other column of the entry keeps resolving through the
    /// alias.
    pub(crate) fn has_duplicate_column(&self, column: &str) -> bool {
        self.schema.columns.iter().filter(|c| c.name == column).count() > 1
    }

    /// Is a reference to `column` over this entry — `s.id`, the bare `id`, or
    /// the `s.*` that would emit it — refused with 42702 because the entry
    /// carries the name twice? Only when an AUTHOR-WRITTEN select list named
    /// both of them: see [`Self::written_select_list`] for why a duplicate the
    /// ENGINE or the CATALOG produced resolves to the first slot instead
    /// (GH#29 c12, M1/M2 — refusing it regressed v4.31.1, which answers).
    ///
    ...
    /// it ambiguous; so does a reference to a MATERIALIZED VIEW whose stored
    /// schema repeats a name. That is v4.31.1's behaviour and the engine-wide
    ...
    pub(crate) fn duplicate_is_ambiguous(&self, column: &str) -> bool {
        self.written_select_list && self.has_duplicate_column(column)
    }
            ...
            None => {
                ...
                for scope in self.levels.iter().rev() {
                    if let Some(entry) = scope.entries.iter().find(|e| e.has_column(column)) {
                        // An AUTHOR-WRITTEN select list NAMES the column
                        // twice (a sub-select whose output repeats it): the
                        // bare spelling is as ambiguous as the qualified one —
                        // PostgreSQL 42702, never the first slot. A duplicate
                        // a WILDCARD expanded, or one a stored relation's own
                        // schema carries, resolves to the first slot
                        // (c12, M1/M2).
                        if entry.duplicate_is_ambiguous(column) {
                            return Resolution::Ambiguous;
                        }
...
mod tests {
    ...
    /// GH#29 (c12, M1/M2/M3). A name the entry carries TWICE is 42702 only
    /// when an AUTHOR-WRITTEN select list named both of them. The SAME schema
    /// resolves to the first slot when the list was entirely wildcards, and
    /// when it is a STORED relation's own schema (a base table or a
    /// materialized view, the default) — that is the whole rule, and it is
    /// decided here, not by the shape of the join.
    #[test]
    fn a_duplicate_is_ambiguous_only_when_the_select_list_wrote_it_twice() {
        let written = derived("s", &["id", "a", "id", "b"]).with_written_select_list(true);
        let expanded = derived("j", &["id", "a", "id", "b"]);
        ...
        // M2: a STORED relation — a base table or a MATERIALIZED VIEW whose
        // schema repeats a name — has no author-written select list at the
        // reference site at all, so it can never be this ambiguity. The
        // default carries that; no call site has to remember to say so.
        let stored = base(None, "m", &["id", "a", "id", "b"]);
        assert!(stored.has_duplicate_column("id"), "the SCHEMA is identical");
        assert!(
            !stored.duplicate_is_ambiguous("id"),
            "an MV's stored duplicate is not a name the author wrote twice"
        );
    ...
    /// The same, for a STAMPED entry (a CTE, view, aliased derived table or
    /// materialized-view scan): the reference keeps its runtime qualifier and
    /// the executor's qualified lookup answers with the first slot of that
    /// name.
    #[test]
    fn a_wildcard_expanded_duplicate_resolves_through_a_runtime_qualifier() {
        let mut stack = ScopeStack::default();
        stack.push();
        stack.record(RangeEntry::new(
            vec!["j".to_string()],
            vec![],
            Some("j".to_string()),
            schema(&["id", "a", "id", "b"]),
