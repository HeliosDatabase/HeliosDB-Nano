...
pub struct Planner<'a> {
    ...
    /// CTE schemas in scope (name -> (schema, author-written select list))
    /// - uses RefCell for interior mutability. The flag travels with the
    /// schema so the two can never disagree; see
    /// `scope::RangeEntry::written_select_list` for what it decides
    /// (GH#29 c12).
    ...
    /// GH#29 (c12, M1/M2/M3): set by [`Self::table_factor_to_plan_inner`] on
    /// the success path when the FROM item it just planned is a CTE, view or
    /// derived table, and TAKEN (cleared) by [`Self::table_factor_to_plan`]
    /// one line later, which hands it to [`Self::record_range_entry`]. It says
    /// whether that item's columns come from a select list the AUTHOR WROTE
    /// OUT; the default — `false` — is what every OTHER FROM shape (a base
    /// table, a MATERIALIZED VIEW, a table function) correctly reports without
    /// naming it. A `Cell`, not a stack: a nested factor planned inside this
    /// one (a view inside a derived table's body) sets and clears the flag
    /// through its OWN `table_factor_to_plan` before this branch sets its own,
    /// so nothing can leak outward.
    written_select_list_of_last_factor: Cell<bool>,
}
...
impl<'a> Planner<'a> {
    ...
    pub fn new() -> Self {
        Self {
            ...
            written_select_list_of_last_factor: Cell::new(false),
        }
    }
    ...
    /// GH#29 (c12): does this CTE's output column list come from a select
    /// list the author WROTE OUT? `false` for a name that is not a CTE in
    /// scope — the caller has already established that it is one.
    fn cte_writes_out_its_select_list(&self, name: &str) -> bool {
        self.cte_schemas
            .borrow()
            .get(name)
    ...
    /// Add a CTE to scope. `written_select_list` records whether its select
    /// list names columns of its own (GH#29 c12); the placeholder schemas a
    /// RECURSIVE CTE pre-registers carry the real body's answer and are
    /// overwritten by the real body below.
    fn add_cte(&self, name: String, schema: Arc<Schema>, written_select_list: bool) {
        self.cte_schemas
            .borrow_mut()
            .insert(name, (schema, written_select_list));
    }

    /// GH#29 (c12, M1/M2/M3): record whether the FROM item just planned takes
    /// its columns from an author-written select list (see
    /// [`Self::written_select_list_of_last_factor`]). An ASSIGNMENT, not a
    ...
    fn set_written_select_list_of_factor(&self, written_select_list: bool) {
        self.written_select_list_of_last_factor.set(written_select_list);
    }

    /// Read and CLEAR the flag [`Self::set_written_select_list_of_factor`]
    /// sets.
    fn take_written_select_list_of_factor(&self) -> bool {
        self.written_select_list_of_last_factor.replace(false)
    }

    /// GH#29 (c12, M1/M3): does this query's output column LIST name columns
    /// the AUTHOR WROTE? True unless the select list is ENTIRELY wildcards —
    /// one or more `*` / `q.*` items and nothing else — because only then did
    /// the engine, rather than the author, decide how many columns of each
    /// name the output carries.
    ///
    /// A list that MIXES the two (`SELECT *, b.id`, `SELECT a.id, b.id, *`)
    /// is written-out: the author named `b.id` beside a `*` that already
    /// carries it, which is a genuine PostgreSQL ambiguity and which v4.31.1
    /// refused. Turning that into a silent answer is the one direction this
    /// rule may never take (c12, M3).
    ///
    /// A set operation takes its output names from its LEFT side
    /// (PostgreSQL), so that is the side consulted.
    fn query_writes_out_its_select_list(query: &Query) -> bool {
        Self::set_expr_writes_out_its_select_list(query.body.as_ref())
    }

    fn set_expr_writes_out_its_select_list(set_expr: &SetExpr) -> bool {
        match set_expr {
            SetExpr::Select(select) => {
                let all_wildcards = !select.projection.is_empty()
                    && select
                        .projection
                        .iter()
                        .all(|item| matches!(item, SelectItem::Wildcard(_) | SelectItem::QualifiedWildcard(..)));
                !all_wildcards
            }
            SetExpr::Query(q) => Self::set_expr_writes_out_its_select_list(q.body.as_ref()),
            SetExpr::SetOperation { left, .. } => Self::set_expr_writes_out_its_select_list(left),
            // `VALUES`, `TABLE t`, and the DML bodies: no select list was
            // written at all, so this is never the author-named ambiguity —
            // the same answer a base-table entry gives (c12, M2). Their output
            // names (`column1`, `column2`, …, or the catalog's) cannot repeat
            // anyway.
            _ => false,
        }
    }

    /// GH#29 (c12, M1/M3): the same question for a parsed VIEW body, which is
    /// a whole statement. Anything but a query has no select list.
    fn statement_writes_out_its_select_list(stmt: &Statement) -> bool {
        match stmt {
            Statement::Query(query) => Self::query_writes_out_its_select_list(query),
            _ => false,
        }
    }
    ...
    ///
    /// `written_select_list` says whether this item's output column list was
    /// written out by the author (GH#29 c12, M1/M2/M3); it decides whether a
    /// name the entry carries twice makes a reference 42702 — see
    /// `scope::RangeEntry::duplicate_is_ambiguous`. A base table, a
    /// MATERIALIZED VIEW and a table function are always `false`: their
    /// columns come from the catalog, not from a select list in this
    /// statement.
    fn record_range_entry(&self, table_factor: &TableFactor, plan: &LogicalPlan, written_select_list: bool) {
        ...
        let entry = match plan {
            ...
            // refused with 42702 by the scope (c3, m3; c12 M1/M2 exempt a
            // duplicate a wildcard expanded and one a stored relation's own
            // schema carries), every other column resolves through the alias.
            _ => {
        ...
        self.scopes
            .borrow_mut()
            .record(entry.with_written_select_list(written_select_list));
    }
    ...
    /// input column. The scope then refuses (42702) a reference to a name the
    /// entry carries twice — bare, qualified or `s.*` — ONLY when an
    /// AUTHOR-WRITTEN select list named both of them (GH#29 c12, M1/M3); a
    /// duplicate a wildcard expanded reads the first slot, as v4.31.1
    /// answered. Every other column of the entry resolves through the alias.
        ...
        match lookup {
            ...
            EntryLookup::Found { level, position } => {
                ...
                for column in &entry.schema.columns {
                    ...
                    // GH#29 (c3, m3; c12 M1): `s.*` over an entry whose
                    // AUTHOR-WRITTEN select list carries a name twice would
                    // emit `s.id` twice and read the first slot for both —
                    // refused exactly like the written `s.id` is.
                    //
                    // Over an entry the ENGINE or the CATALOG built — a
                    // wildcard-expanded CTE / view / derived table, or a
                    // MATERIALIZED VIEW whose stored schema repeats a name —
                    // it is NOT refused: v4.31.1 answers `SELECT j.*` there
                    // with the un-merged pair (`[1, 11, 1, 111]`), which is
                    // the same arity a bare `SELECT *` gives today, so
                    // refusing the qualified spelling while allowing the bare
                    // one would be both a regression against the shipped
                    // release and an inconsistency (c12, M1).
                    if entry.duplicate_is_ambiguous(&column.name) {
                        return Err(super::scope::ambiguous_column(&column.name));
                    }
                    out.push((
                        LogicalExpr::Column {
    ...
    fn query_to_plan(&self, query: Query) -> Result<LogicalPlan> {
        ...
        let (cte_plans, is_recursive) = if let Some(with_clause) = query.with {
            ...
            for cte in with_clause.cte_tables {
                ...
                // GH#29 (c12, M1/M3): the CTE body's select-list shape, read
                // before the query is consumed by the planner below.
                let cte_writes_out_its_select_list = Self::query_writes_out_its_select_list(&cte.query);
                ...
                if is_recursive && !column_aliases.is_empty() {
                    ...
                    self.add_cte(cte_name.clone(), schema, cte_writes_out_its_select_list);
                ...
                self.add_cte(cte_name.clone(), cte_schema, cte_writes_out_its_select_list);
    ...
    fn select_to_plan(&self, select: Select) -> Result<LogicalPlan> {
        ...
        if has_aggregates || has_group_by {
            ...
            // output shape. A WRITTEN `id`, a written `j.id` and `j.*` over
            // such an entry are NOT refused either (c12, M1/M2): v4.31.1
            // answers all three. What keeps its 42702 is a name an
            // AUTHOR-WRITTEN select list carries twice — including a list that
            // MIXES a wildcard with a written item (c12, M3).
    ...
    fn table_factor_to_plan(&self, table_factor: &TableFactor) -> Result<LogicalPlan> {
        ...
        // the same clause cannot inherit it (GH#29 c12, M1/M2/M3).
        let written_select_list = self.take_written_select_list_of_factor();
        self.record_range_entry(table_factor, &plan, written_select_list);
        Ok(plan)
    }
        ...
        match table_factor {
            TableFactor::Table { name, alias, args, .. } => {
                ...
                if let Some(cte_schema) = self.get_cte_schema(&table_name) {
                    ...
                    let table_alias = alias.as_ref().map(|a| Self::normalize_ident(&a.name));
                    // GH#29 (c12, M1/M3): carry the CTE's select-list shape to
                    // the range entry. Read from the SAME map the schema came
                    // from, so the two can never disagree.
                    self.set_written_select_list_of_factor(self.cte_writes_out_its_select_list(&table_name));
                ...
                if let Some(catalog) = self.catalog {
                    ...
                    if let Some(view_key) = view_key {
                        ...
                        // GH#29 (c12, M1/M3): read the view BODY's select-list
                        // shape before the statement is consumed.
                        let view_writes_out_its_select_list = Self::statement_writes_out_its_select_list(&stmt);
                        ...
                        self.set_written_select_list_of_factor(view_writes_out_its_select_list);
                        return Ok(stamped);
                    }
                }
            ...
            TableFactor::Derived {
                ...
                let subquery_writes_out_its_select_list = Self::query_writes_out_its_select_list(subquery);
                ...
                //
                // GH#29 (c12, M1/M3): the sub-select's own select-list shape
                // is noted AFTER its body is planned — the body's own FROM
                // items each set and cleared the flag through their own
                ...
                self.set_written_select_list_of_factor(subquery_writes_out_its_select_list);
                Ok(stamped)
            }
...
mod tests {
    ...
    /// GH#29 (c12, M3). The flag that DISARMS the 42702 for a name an entry
    /// carries twice turns on ONE question: is the select list ENTIRELY
    /// wildcards? A list that mixes a wildcard with a written item is
    /// author-written, keeps its 42702, and must not be disarmed — v4.31.1
    /// refused `SELECT s.id FROM (SELECT *, b.id FROM a JOIN b …) s`, and
    /// turning an error into a silent answer is the one direction this rule
    /// may never take (candidate 11 did; this is the pin against it).
    #[test]
    fn only_an_entirely_wildcard_select_list_is_not_author_written() {
        let parser = Parser::new();
        let written = |sql: &str| match parser.parse_one(sql).expect("parse") {
            Statement::Query(query) => Planner::query_writes_out_its_select_list(&query),
            _ => panic!("expected a query: {sql}"),
        };

        // Entirely wildcards — the engine, not the author, shaped the output.
        assert!(!written("SELECT * FROM a NATURAL JOIN b"));
        assert!(!written("SELECT a.*, b.* FROM a JOIN b ON a.id = b.id"));
        assert!(!written("SELECT * FROM a JOIN b USING (id)"));

        // MIXED — the author named `b.id` beside a `*` that already carries
        // it. This is M3: candidate 11 called these wildcard-output.
        assert!(written("SELECT *, b.id FROM a JOIN b ON a.id = b.id"));
        assert!(written("SELECT a.id, b.id, * FROM a JOIN b ON a.id = b.id"));
        assert!(written("SELECT a.*, b.id FROM a JOIN b ON a.id = b.id"));

        // Written out, no wildcard at all.
        assert!(written("SELECT a.id, b.id FROM a JOIN b ON a.id = b.id"));

        // A set operation takes its output names from its LEFT side.
        assert!(!written(
            "SELECT * FROM a NATURAL JOIN b UNION ALL SELECT a.id, a.v FROM a"
        ));
        assert!(written("SELECT a.id FROM a UNION ALL SELECT * FROM b"));
    }
