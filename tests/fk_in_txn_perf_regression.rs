...
/// Edge case 4: regular UPDATE refreshes ART inside the transaction.
...
fn in_txn_update_parent_pk_then_insert_child_fk_to_new_succeeds() {
    ...
    db.execute("BEGIN").expect("begin");
    db.execute("UPDATE p SET id = 42 WHERE id = 1")
        .expect("update parent pk in txn");
...
fn in_txn_insert_then_rollback_clears_art() {
    ...
    db.execute("BEGIN").expect("begin");
    db.execute("INSERT INTO p (id) VALUES (777)")
        .expect("insert parent in txn");
    ...
    assert!(r.is_err(), "ART must not retain rolled-back insertions; got {r:?}");
}
...
fn in_txn_bulk_fk_inserts_under_5s() {
    ...
    for i in 1..=N_CHILDREN {
        ...
        db.execute(&format!("INSERT INTO c (id, parent_id) VALUES ({i}, {parent_id})"))
            .expect("child insert");
    }
    let dt = t0.elapsed();
    db.execute("COMMIT").expect("commit");
