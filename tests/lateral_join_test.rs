...
fn test_subquery_in_from_simple() {
    ...
    db.execute("CREATE TABLE orders (id INT PRIMARY KEY, amount INT)")
        .unwrap();
    db.execute("INSERT INTO orders (id, amount) VALUES (1, 100), (2, 200), (3, 300)")
        .unwrap();

    // Test simple subquery in FROM clause without alias qualification
    let results = db
        .query(
            "SELECT * FROM (SELECT id, amount FROM orders) AS sub WHERE amount > 150",
            &[],
        )
        .unwrap();
...
fn test_cross_join_with_subquery() {
    ...
    db.execute("CREATE TABLE products (id INT PRIMARY KEY, pname TEXT, price INT)")
        .unwrap();
    db.execute("INSERT INTO products (id, pname, price) VALUES (1, 'Widget', 10), (2, 'Gadget', 20)")
        .unwrap();
...
fn test_inner_join_with_subquery() {
    ...
    db.execute("CREATE TABLE departments (dept_id INT PRIMARY KEY, dept_name TEXT)")
        .unwrap();
    db.execute("INSERT INTO departments (dept_id, dept_name) VALUES (1, 'Engineering'), (2, 'Sales')")
        .unwrap();

    db.execute("CREATE TABLE employees (id INT PRIMARY KEY, name TEXT, dept_id INT)")
        .unwrap();
    db.execute("INSERT INTO employees (id, name, dept_id) VALUES (1, 'Alice', 1), (2, 'Bob', 1), (3, 'Charlie', 2)")
        .unwrap();
...
fn test_nested_subqueries() {
    ...
    db.execute("INSERT INTO data (id, value) VALUES (1, 10), (2, 20), (3, 30)")
        .unwrap();
...
fn test_subquery_with_aggregation() {
    ...
    db.execute("CREATE TABLE sales (id INT PRIMARY KEY, product TEXT, amount INT)")
        .unwrap();
    db.execute("INSERT INTO sales (id, product, amount) VALUES (1, 'A', 100), (2, 'A', 200), (3, 'B', 150)")
        .unwrap();
...
fn test_subquery_with_filter() {
    ...
    db.execute("CREATE TABLE items (id INT PRIMARY KEY, category TEXT, price INT)")
        .unwrap();
    db.execute("INSERT INTO items (id, category, price) VALUES (1, 'A', 10), (2, 'A', 20), (3, 'B', 30)")
        .unwrap();
...
fn test_subquery_select_specific_columns() {
    ...
    // Create table
    db.execute("CREATE TABLE records (id INT PRIMARY KEY, col1 TEXT, col2 INT, col3 TEXT)")
        .unwrap();
    db.execute("INSERT INTO records (id, col1, col2, col3) VALUES (1, 'a', 10, 'x'), (2, 'b', 20, 'y')")
        .unwrap();

    // Test subquery that selects specific columns
    let results = db
        .query("SELECT col1, col2 FROM (SELECT col1, col2 FROM records) AS sub", &[])
        .unwrap();
    assert_eq!(results.len(), 2);
