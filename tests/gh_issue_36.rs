...
fn is_schema_change(op: &WalOperation) -> bool {
    match op {
        ...
        | WalOperation::DropConstraint { .. } => true,
    }
}
...
fn params_family_alter_table_column_ddl_is_rejected_loudly_not_silently_unlogged() {
    for stmt in [
        ...
        let (err, ops) = emitted_after_rejection(&[T], stmt, Family::Params);
        assert!(
            !err.is_empty(),
            "`{stmt}` SUCCEEDED on the params family — ALTER TABLE parity has landed. Delete this \
             control and assert the WAL entry on BOTH families instead (this file's whole point \
             is that a schema change a replica cannot see is worse than one that errors). \
             Entries: [{}]",
            describe(&ops)
        );
