...
//! | `SELECT id FROM (SELECT a.id, b.id …) s` (a name one derived entry's WRITTEN-OUT select list carries twice) | first slot | 42702, bare and qualified alike — but only for a WRITTEN-OUT list: a duplicate a WILDCARD expanded is our own un-merged output and resolves to the first slot, as v4.31.1 did (c11 M1) |
...
fn a_from_chain_naming_one_relation_twice_is_not_a_tautology() {
    ...
    // `a_written_reference_to_a_duplicated_output_name_is_still_ambiguous`.)
    for sql in [
...
// PostgreSQL carries it once. Candidate 11 (M1) finished that: a WRITTEN
// `j.id` over such a side returns the row too, as v4.31.1 does — only `j.*`
// is still refused, and the candidate-11 block says why.
//
...
/// What is STILL refused over a wildcard-expanded side that carries the shared
/// name twice, after candidate 11 (M1) let the written `j.id` through: `j.*`,
/// and only `j.*`.
///
/// Candidate 10 refused the written `id` / `j.id` here as well. That was a
/// REGRESSION against v4.31.1, which returns the row, and it is fixed in the
/// candidate-11 block below — the assertions those statements now earn are
/// here, as the other half of this pin.
#[test]
fn a_qualified_wildcard_over_a_duplicated_output_name_is_still_ambiguous() {
    let db = merged_side_db();
    let dup = "(SELECT * FROM y1 NATURAL JOIN y2) j";
    // OVER-REFUSAL, stated (m4c): PostgreSQL ACCEPTS `SELECT j.*` here,
    // because ITS `j` carries `id` once — it merges the shared output column
    // of a NATURAL join and we do not (sprinter 781f55ba534d). Ours carries it
    // twice, so `j.*` would emit `j.id` TWICE under a qualifier that can only
    // read the FIRST slot and the second column would silently repeat the
    // first. Refusing is fail-closed until the column is merged. The WRITTEN
    // `j.id` needs only one slot and is NOT refused.
    assert_refused(&db, &format!("SELECT j.* FROM {dup}"), AMBIGUOUS);
