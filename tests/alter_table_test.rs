...
fn test_alter_table_drop_column() {
    ...
    db.execute("CREATE TABLE employees (id INT PRIMARY KEY, name TEXT, salary INT)")
        .unwrap();
    db.execute("INSERT INTO employees (id, name, salary) VALUES (1, 'Alice', 50000)")
        .unwrap();
...
fn test_alter_table_rename_column() {
    ...
    db.execute("CREATE TABLE accounts (id INT PRIMARY KEY, username TEXT)")
        .unwrap();
    db.execute("INSERT INTO accounts (id, username) VALUES (1, 'johndoe')")
        .unwrap();

    // Rename column
    db.execute("ALTER TABLE accounts RENAME COLUMN username TO login")
        .unwrap();
...
fn test_alter_table_rename() {
    ...
    // Create table with data
    db.execute("CREATE TABLE old_table (id INT PRIMARY KEY, value TEXT)")
        .unwrap();
    db.execute("INSERT INTO old_table (id, value) VALUES (1, 'test')")
        .unwrap();
...
fn test_alter_table_multi_add_two_columns() {
    ...
    db.execute("ALTER TABLE products ADD COLUMN price INT, ADD COLUMN stock INT")
        .unwrap();
    ...
    db.execute("INSERT INTO products (id, name, price, stock) VALUES (2, 'Gadget', 999, 50)")
        .unwrap();
...
fn test_alter_table_multi_add_and_drop_column() {
    ...
    db.execute("CREATE TABLE employees (id INT PRIMARY KEY, name TEXT, temp_col TEXT)")
        .unwrap();
    db.execute("INSERT INTO employees (id, name, temp_col) VALUES (1, 'Alice', 'remove_me')")
        .unwrap();

    // Add a column and drop a different column in one statement
    db.execute("ALTER TABLE employees ADD COLUMN email TEXT, DROP COLUMN temp_col")
        .unwrap();
...
fn test_alter_table_multi_rename_and_add_column() {
    ...
    db.execute("ALTER TABLE accounts RENAME COLUMN username TO login, ADD COLUMN email TEXT")
        .unwrap();
...
fn test_alter_table_multi_three_operations() {
    ...
    db.execute("CREATE TABLE data (id INT PRIMARY KEY, col_a TEXT, col_b TEXT)")
        .unwrap();
    db.execute("INSERT INTO data (id, col_a, col_b) VALUES (1, 'a', 'b')")
        .unwrap();

    // Three operations: rename col_a, drop col_b, add col_c
    db.execute("ALTER TABLE data RENAME COLUMN col_a TO alpha, DROP COLUMN col_b, ADD COLUMN col_c INT")
        .unwrap();
...
fn test_alter_table_multi_error_in_second_op() {
    ...
    let result = db.execute("ALTER TABLE items ADD COLUMN price INT, DROP COLUMN nonexistent");
    assert!(result.is_err());
...
fn test_alter_table_multi_with_if_not_exists() {
    ...
    db.execute("CREATE TABLE settings (id INT PRIMARY KEY, key TEXT, value TEXT)")
        .unwrap();

    // First operation: add column that already exists (with IF NOT EXISTS -- should be no-op)
    // Second operation: add a new column
    db.execute("ALTER TABLE settings ADD COLUMN IF NOT EXISTS value TEXT, ADD COLUMN description TEXT")
        .unwrap();

    // Should have 4 columns: id, key, value, description
    // Table is empty but we can check by inserting
    db.execute("INSERT INTO settings (id, key, value, description) VALUES (1, 'theme', 'dark', 'UI theme')")
        .unwrap();
...
fn test_alter_table_multi_with_if_exists_drop() {
    ...
    // Second operation: add a new column
    db.execute("ALTER TABLE logs DROP COLUMN IF EXISTS phantom, ADD COLUMN severity INT")
        .unwrap();

    // Should have 3 columns: id, msg, severity
    db.execute("INSERT INTO logs (id, msg, severity) VALUES (1, 'test', 3)")
        .unwrap();
...
fn test_alter_table_multi_add_columns_with_defaults() {
    ...
    db.execute("ALTER TABLE config ADD COLUMN enabled BOOLEAN DEFAULT TRUE, ADD COLUMN retries INT DEFAULT 3")
        .unwrap();

    let results = db
        .query("SELECT enabled, retries FROM config WHERE id = 1", &[])
        .unwrap();
    assert_eq!(results.len(), 1);
