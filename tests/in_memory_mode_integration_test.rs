...
fn test_in_memory_no_persistence() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Table from previous in-memory instance should not exist"
    );
    ...
    for i in 0..10 {
        ...
        handles.push(thread::spawn(move || {
            ...
            let session = db_clone
                .create_session(
                    &format!("user_{}", i),
                    heliosdb_nano::session::IsolationLevel::ReadCommitted,
                )
                .unwrap();
            db_clone
                .execute_in_session(session, &format!("INSERT INTO concurrent VALUES ({})", i))
                .unwrap();
        }));
    }
