...
async fn test_server_crud_operations() {
    ...
    // Insert
    client
        .execute("INSERT INTO users VALUES ($1, $2)", &[&1i32, &"Alice"])
        .await
    ...
    // Update
    client
        .execute("UPDATE users SET name = $1 WHERE id = $2", &[&"Bob", &1i32])
        .await
    ...
    // Delete
    client
        .execute("DELETE FROM users WHERE id = $1", &[&1i32])
        .await
...
async fn test_server_transaction_handling() {
    ...
    tx.execute("UPDATE accounts SET balance = 200 WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    // Test Commit
    let tx = client.transaction().await.unwrap();
    tx.execute("UPDATE accounts SET balance = 300 WHERE id = 1", &[])
        .await
        .unwrap();
...
async fn test_server_concurrent_clients() {
    ...
    // Initialize DB
    {
        ...
        client
            .execute("CREATE TABLE concurrent (id INT PRIMARY KEY)", &[])
            .await
            .unwrap();
    }
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            client
                .execute("INSERT INTO concurrent VALUES ($1)", &[&(i as i32)])
                .await
                .unwrap();
        }));
    }
