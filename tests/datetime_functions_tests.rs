...
mod datetime {
    ...
    fn to_char_basic_iso_date() {
        let db = db();
        let r = db
            .query("SELECT TO_CHAR(MAKE_DATE(2026, 4, 28), 'YYYY-MM-DD') AS d", &[])
            .unwrap();
    ...
    fn to_char_month_names() {
        let db = db();
        let r = db
            .query("SELECT TO_CHAR(MAKE_DATE(2026, 4, 28), 'Mon DD YYYY') AS s", &[])
            .unwrap();
        assert_eq!(to_str(&r[0].values[0]), "Apr 28 2026");

        let r2 = db
            .query("SELECT TO_CHAR(MAKE_DATE(2026, 4, 28), 'MON') AS s", &[])
            .unwrap();
    ...
    fn to_char_weekday_names() {
        ...
        let db = db();
        let r = db
            .query("SELECT TO_CHAR(MAKE_DATE(2026, 4, 28), 'Day') AS s", &[])
            .unwrap();
    ...
    fn to_char_quarter_and_iso_week() {
        let db = db();
        let r = db
            .query("SELECT TO_CHAR(MAKE_DATE(2026, 4, 28), 'Q-IW') AS s", &[])
            .unwrap();
    ...
    fn make_date_round_trip() {
        let db = db();
        let r = db
            .query("SELECT TO_CHAR(MAKE_DATE(2000, 1, 1), 'YYYY-MM-DD') AS s", &[])
            .unwrap();
    ...
    fn make_date_invalid_rejected() {
        let db = db();
        let res = db.query("SELECT MAKE_DATE(2026, 13, 1) AS bad", &[]);
    ...
    fn date_aggregation_smoke() {
        let db = db();
        db.execute("CREATE TABLE events (id INT PRIMARY KEY, ts TIMESTAMP, body TEXT)")
            .unwrap();
        db.execute(
